...
public class HuffmanListFundTest {
	...
	@Test
	public void testFileArgumentConstructor() {
		HuffmanList hList = new HuffmanList("file.txt");
		Iterator<HuffmanNode> iter = hList.iterator();

		assertTrue(
			"HuffmanList constructed from file.txt should not be empty",
			iter.hasNext());

		HuffmanNode hNode = iter.next();
		assertEquals(
			"The first element of HuffmanList should contain byte '\n'",
			(byte)'\n', hNode.b);
		assertEquals(
			"The first element of HuffmanList should be with count 1",
			1, hNode.count);
		assertTrue(
			"HuffmanList constructed from file.txt should have more that 1 elements",
			iter.hasNext());

		hNode = iter.next();
		assertEquals(
			"The second element of HuffmanList should contain byte 'c'",
			(byte)'c', hNode.b);
		assertEquals(
			"The second element of HuffmanList should be with count 2",
			2, hNode.count);
	...
	public void testTwoArgumentContsructorDuplicateException() {
    	@SuppressWarnings("unused")
		HuffmanList hList = new HuffmanList(new byte[] {(byte)'a', (byte)'b', (byte)'a'}, new int[] {3, 2, 1});
	}
	
	@Test(expected = IllegalArgumentException.class)
	public void testTwoArgumentContsructorNegativeException() {
		@SuppressWarnings("unused")
		HuffmanList hList = new HuffmanList(new byte[] {(byte)'a', (byte)'b', (byte)'c'}, new int[] {3, 2, -1});
	}
	...
	public void testTwoArgumentConstructor() {
		...
		assertTrue(
			"HuffmanList constructed from byte array ['a', 'b', 'c'] and counts [3, 2, 1] should not be empty",
			iter.hasNext());

		HuffmanNode hNode = iter.next();
		assertEquals(
			"The first element of HuffmanList should contain byte 'c'",
			(byte)'c', hNode.b);
		assertEquals(
			"The first element of HuffmanList should be with count 1",
			1, hNode.count);
		assertTrue(
			"HuffmanList constructed from byte array ['a', 'b', 'c'] and counts [3, 2, 1] should have 3 elements",
			iter.hasNext());

		hNode = iter.next();
		assertEquals(
			"The second element of HuffmanList should contain byte 'b'",
			(byte)'b', hNode.b);
		assertEquals(
			"The second element of HuffmanList should be with count 2",
			2, hNode.count);
		iter.next();
		assertFalse(
			"HuffmanList constructed from byte array ['a', 'b', 'c'] should only have 3 elements",
			iter.hasNext());
	}
